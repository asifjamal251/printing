                    ...
                    <div class="card-body">
                        ...
                        <table id="datatable" class="datatable table-sm border-secondary table-hover table table-bordered nowrap align-middle" style="width:100%">
                            ...
                            <tr>
                                ...
                                <th>Status</th>
                                <th>Timer</th>
                                @can(['edit_spot_uv','delete_spot_uv', 'read_spot_uv', 'change_status_spot_uv'])
                                  <th>Action</th>
                                @endcan
                            </tr>
                        </thead>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group{{ $errors->has('oprator') ? ' has-error' : '' }}">
                    {!! Form::select('oprator', App\Models\ModuleUser::where('module_id', 6)->pluck('name', 'id'), null, ['id' => 'oprator', 'class' => 'form-control form-control-sm', 'placeholder' => 'Choose Oprator']) !!}
...
$(document).ready(function(){
    ...
    "columns": [
        ...
        { "data": "spot_uv_sheets",
            render: function(data, type, row) {
                if(row['status_id'] == 5){
                    return row['spot_uv_sheets'];
                }else{
                    return '<input  data-total="'+row['total_sheet']+'" data-id="'+row['id']+'" type="text" value="'+row['spot_uv_sheets']+'" value="'+row['spot_uv_sheets']+'" class="form-control form-control-sm coating-input" name="spot_uv_sheets[]" placeholder="Coated Sheets" style="max-width:100px;">';
                }
        ...
        { "data": "timer" },
        {
            ...
            render: function(data, type, row) {
                if (type === 'display') {
                    ...
                    @can(['edit_spot_uv','delete_spot_uv','read_spot_uv', 'change_status_spot_uv'])
                    btn+='<li><a class="dropdown-item edit-item-btn" onclick="updateTimer(\'{{ route('admin.job-card.timer.content') }}\',{machine:7, id:'+row['id']+',job_card_id:'+row['job_card_id']+'})" href="javascript:void(0);"><i class="ri-alarm-line align-bottom me-2 text-muted"></i> Timer</a></li>';

                    @can('change_status_spot_uv')
                        if(row['status_id'] == 2){
                            btn+='<li><a onclick="updateData(\'{{ route('admin.spot-uv.changeStatus') }}\',{status:1, id:'+row['id']+',job_card_id:'+row['job_card_id']+'})" class="dropdown-item edit-item-btn" href="javascript:void(0);"><i class="ri-check-double-line align-bottom me-2 text-muted"></i> Completed</a></li>';
                        }else{
                            btn+='<li><a onclick="updateData(\'{{ route('admin.spot-uv.changeStatus') }}\',{status:2, id:'+row['id']+',job_card_id:'+row['job_card_id']+'})" class="dropdown-item edit-item-btn" href="javascript:void(0);"><i class="bx bx-x align-bottom me-2 fs-24 text-muted"></i> Cancel</a></li>';
                        }
                    @endcan
...
$('body').on('click', '.filters', function(){
    ...
    $('body').on('change', '.coating-input', function(){
        var spot_uv_sheets = parseInt($(this).val());
        var total_sheets = parseInt($(this).attr('data-total'));
        var id = $(this).attr('data-id');
        
        if(spot_uv_sheets < total_sheets){
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: '{{ route('admin.spot-uv.update', '') }}/'+id,
                data: { 'spot_uv_sheets': spot_uv_sheets, '_method': 'PUT', '_token': '{{ csrf_token() }}' },
            ...
            $(this).val('')
            Toastify({
                text: 'Sheets always less than Total Sheets.',
                duration: 3000,
                close: true,
                gravity: "top", // `top` or `bottom`
                position: "right", // `left`, `center` or `right`
