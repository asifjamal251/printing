        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group{{ $errors->has('oprator') ? ' has-error' : '' }}">
                    {!! Form::select('oprator', App\Models\ModuleUser::where('module_id', 2)->pluck('name', 'id'), null, ['id' => 'oprator', 'class' => 'form-control form-control-sm', 'placeholder' => 'Choose Oprator']) !!}
...
$(document).ready(function(){
    ...
    "columns": [
        ...
        { "data": "coated_sheets",
            render: function(data, type, row) {
                if(row['status_id'] == 5){
                    return row['coated_sheets'];
                }else{
                    return '<input data-total="'+row['total_sheet']+'"  data-id="'+row['id']+'" type="text" class="form-control form-control-sm coating-input" name="coated_sheets[]" value="'+row['coated_sheets']+'" placeholder="Coated Sheets" style="max-width:100px;">';
                }
                ...
                if (type === 'display') {
                    ...
                    @can('change_status_chemical_coating')
                        if(row['status_id'] == 2){
                            ...
                            btn+='<li><a onclick="updateData(\'{{ route('admin.chemical-coating.changeStatus') }}\',{status:2,id:'+row['id']+',job_card_id:'+row['job_card_id']+'})" class="dropdown-item edit-item-btn" href="javascript:void(0);"><i class="ri-check-double-line align-bottom me-2 text-muted"></i> Cancel</a></li>';
                        }
                    @endcan


                    // @can('edit_chemical_coating')
...
$('body').on('click', '.filters', function(){
    ...
    $('body').on('change', '.selectUser', function(){
    ...
    $('body').on('change', '.coating-input', function(){
        var coated_sheets = parseInt($(this).val());
        var total_sheets = parseInt($(this).attr('data-total'));
        var id = $(this).attr('data-id');
        
        if(coated_sheets < total_sheets){
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: '{{ route('admin.chemical-coating.update', '') }}/'+id,
                data: { 'coated_sheets': coated_sheets, '_method': 'PUT', '_token': '{{ csrf_token() }}' },
                success: function (response) {
                    Toastify({
                        text: response.message,
                        duration: 3000,
            ...
            $(this).val('');
            Toastify({
                text: 'Coated Sheets always less than Total Sheets.',
                duration: 3000,
                close: true,
                gravity: "top", // `top` or `bottom`
                position: "right", // `left`, `center` or `right`
