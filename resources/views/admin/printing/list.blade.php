        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group{{ $errors->has('oprator') ? ' has-error' : '' }}">
                    {!! Form::select('oprator', App\Models\ModuleUser::where('module_id', 9)->pluck('name', 'id'), null, ['id' => 'oprator', 'class' => 'form-control form-control-sm', 'placeholder' => 'Choose Oprator']) !!}
...
$(document).ready(function(){
    ...
    "columns": [
        ...
        { "data": "printed_sheet",
            render: function(data, type, row) {
                if(row['status_id'] == 5){
                    return row['printed_sheet'];
                }else{
                    return '<input data-total="'+row['total_sheet']+'"  data-id="'+row['id']+'" type="text" class="form-control form-control-sm printed-sheet-input" value="'+row['printed_sheet']+'" name="printed_sheet[]" placeholder="Printed Sheets" style="max-width:100px;">';
                }
...
$('body').on('change', '.selectUser', function(){
    ...
    $.ajax({
        ...
        url:'{{ route('admin.'.request()->segment(2).'.user.assign', '') }}/'+id,
        data: {'user':user,'_method': 'POST', '_token': '{{ csrf_token() }}'},
    ...
    $('body').on('change', '.printed-sheet-input', function(){
        ...
        if(printed_sheet < total_sheets){
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: '{{ route('admin.printing.update', '') }}/'+id,
