                    ...
                    <div class="card-body">
                        ...
                        <table id="datatable" class="datatable table-sm border-secondary table-hover table table-bordered nowrap align-middle" style="width:100%">
                            ...
                            <tr>
                                ...
                                <th>Status</th>
                                <th>Timer</th>
                                @can(['edit_lamination','delete_lamination', 'read_lamination', 'change_status_lamination'])
                                  <th>Action</th>
                                @endcan
                            </tr>
                        </thead>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group{{ $errors->has('oprator') ? ' has-error' : '' }}">
                    {!! Form::select('oprator', App\Models\ModuleUser::where('module_id', 3)->pluck('name', 'id'), null, ['id' => 'oprator', 'class' => 'form-control form-control-sm', 'placeholder' => 'Choose Oprator']) !!}
...
$(document).ready(function(){
    ...
    "columns": [
        ...
        { "data": "sheet_size" },
        { "data": "total_sheet" },
        { "data": "laminated_sheets",
            render: function(data, type, row) {
                if(row['status_id'] == 5){
                    return  row['laminated_sheets'];;
                }else{
                    return '<input data-total="'+row['total_sheet']+'"  data-id="'+row['id']+'" type="text" class="form-control form-control-sm printed-sheet-input" value="'+row['laminated_sheets']+'" name="laminated_sheets[]" placeholder="Laminated Sheets" style="max-width:100px;">';
                }
...
$('body').on('click', '.filters', function(){
    ...
    $('body').on('change', '.printed-sheet-input', function(){
        ...
        if(printed_sheet < total_sheets){
            $.ajax({
                ...
                url: '{{ route('admin.lamination.update', '') }}/'+id,
