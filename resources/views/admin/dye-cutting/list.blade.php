                    ...
                    <div class="card-body">
                        ...
                        <table id="datatable" class="datatable table-sm border-secondary table-hover table table-bordered nowrap align-middle" style="width:100%">
                            ...
                            <tr>
                                ...
                                <th>Status</th>
                                <th>Timer</th>
                                @can(['edit_dye_cutting','delete_dye_cutting', 'read_dye_cutting', 'change_status_dye_cutting'])
                                  <th>Action</th>
                                @endcan
                            </tr>
                        </thead>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group{{ $errors->has('oprator') ? ' has-error' : '' }}">
                    {!! Form::select('oprator', App\Models\ModuleUser::where('module_id', 7)->pluck('name', 'id'), null, ['id' => 'oprator', 'class' => 'form-control form-control-sm', 'placeholder' => 'Choose Oprator']) !!}
...
$(document).ready(function(){
    ...
    "columns": [
        ...
        { "data": "dye_counter",
            render: function(data, type, row) {
                if(row['status_id'] == 5){
                    return row['dye_counter'];
                }else{
                    return '<input data-total="'+row['total_sheet']+'"  data-id="'+row['id']+'" type="text" class="form-control form-control-sm dye-cutting-input" name="dye_counter[]" value="'+row['dye_counter']+'" placeholder="Dye Counter" style="max-width:100px;">';
                }
...
var table = table2;
 ...
 $('body').on('click', '.filters', function(){
    ...
    $('body').on('change', '.dye-cutting-input', function(){
        ...
        if(sheets < total_sheets){
            $.ajax({
                type: "POST",
                enctype: 'multipart/form-data',
                url: '{{ route('admin.dye-cutting.update', '') }}/'+id,
                data: { 'sheets': sheets, '_method': 'PUT', '_token': '{{ csrf_token() }}' },
                success: function (response) {
                    Toastify({
                        text: response.message,
                        duration: 3000,
