    ...
    <div class="col-md-6 col-sm-12">
        <div class="card">
            <div class="card-header">
                <h6 class="card-title mb-0">Bye Breaking</h6>
            </div>
            <div class="card-body">
                @foreach($job_card->jobCardItems as $job_item)
                {{dd($job_item->POItem)}}
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="form-group {{$errors->has('dye_braking.'.$loop->index.'.carton') ? ' has-error' : '' }}">
                                {!! Form::label('carton', 'Module') !!}
                                {!! Form::select('dye_braking['.$loop->index.'][carton]',  $job_item->POItem, 9, ['class' => 'form-control', 'placeholder' => 'Choose Module']) !!}
                                <small class="text-danger">{{ $errors->first('dye_braking.'.$loop->index.'.carton') }}</small>
                            </div>
                        </div>

                        <div class="col-md-6 col-sm-12">
                            <div class="form-group {{$errors->has('dye_braking.'.$loop->index.'.module_machine') ? ' has-error' : '' }}">
                                {!! Form::label('module_machine', 'Module Machine') !!}
                                {!! Form::select('dye_braking['.$loop->index.'][module_machine]', App\Models\ModuleUser::where('module_id', 9)->pluck('name', 'id'), null, ['class' => 'form-control', 'placeholder' => 'Choose Module Machine']) !!}
                                <small class="text-danger">{{ $errors->first('dye_braking.'.$loop->index.'.module_machine') }}</small>
                            </div>
                        </div>
                    </div>
                @endforeach
