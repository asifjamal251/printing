...
class PastingResource extends JsonResource{
    private function timer($jobCardId, $type){
        $timer = JobCardTimer::where(['machine' => 'Pasting', 'job_card_id' => $jobCardId])->first();
        if($timer){
            if($type == 'Timer'){
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'dye_no'=>@$this->jobCard->dye_details_id?@$this->jobCard->deyDetail->dye_no.'/'.@$this->jobCard->deyDetail->dye_lock:'NEW',        
